...
import { Patient, CIFProfile, HypothesisComparison, ImpactData, GoniometriaValue } from '../types';
import { ChevronLeftIcon, FileDownIcon, LoaderIcon, SparklesIcon, BrainCircuitIcon } from '../components/IconComponents';
                    ...
                    {patient.clinicalRecords.map((record, index) => {
                        ...
                        <details key={record.id} className="bg-white rounded-xl shadow-md border border-slate-200/80 overflow-hidden" open={index === 0}>
                            ...
                            <div className="px-6 pb-6 pt-2 border-t border-slate-200 space-y-6">
                                ...
                                {record.radiology && (record.radiology.imageBase64 || record.radiology.inclinacionRadial) && (
                                    <div>
                                        <h4 className="font-semibold text-slate-700 mt-4 mb-2">Radiografía</h4>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                                            {record.radiology.imageBase64 && <img src={record.radiology.imageBase64} alt="Radiografía del paciente" className="rounded-lg border border-slate-200 w-full" />}
                                            <div className={!record.radiology.imageBase64 ? 'md:col-span-2' : ''}>
                                                <div className="space-y-1 mb-3 bg-slate-50 p-3 rounded-md border">
                                                    ...
                                                    <DetailItem label="Varianza Cubital" value={record.radiology?.varianzaCubital ? `${record.radiology.varianzaCubital} mm` : ''} />
